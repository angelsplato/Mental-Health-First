...
import com.angels.config.KafkaProperties;
...
class BlogKafkaResourceIT {
    ...
    void setup() {
        ...
        BlogKafkaResource kafkaResource = new BlogKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/blog-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/blog-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
